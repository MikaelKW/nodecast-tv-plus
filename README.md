...
The migration test requires Docker. It builds pinned upstream versions, generates temporary secrets and test records, upgrades each disposable data volume into the local Plus image, and removes the test containers, volumes, and baseline images afterward. CI runs this check as part of the Docker build job.
